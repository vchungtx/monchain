...
"$PWD"/build/monchaind keys add $KEY --keyring-backend test --algo "eth_secp256k1"

# Set moniker and chain-id for Ethermint (Moniker can be anything, chain-id must be an integer)
"$PWD"/build/monchaind init $MONIKER --chain-id $CHAINID
...
"$PWD"/build/monchaind add-genesis-account "$("$PWD"/build/monchaind keys show "$KEY" -a --keyring-backend test)" 100000000000000000000aphoton,10000000000000000000stake --keyring-backend test

# Sign genesis transaction
"$PWD"/build/monchaind gentx $KEY 10000000000000000000stake --amount=100000000000000000000aphoton --keyring-backend test --chain-id $CHAINID

# Collect genesis tx
"$PWD"/build/monchaind collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
"$PWD"/build/monchaind validate-genesis

# Start the node (remove the --pruning=nothing flag if historical queries are not needed) in background and log to file
"$PWD"/build/monchaind start --pruning=nothing --rpc.unsafe --json-rpc.address="127.0.0.1:8545" --keyring-backend test > monchaind.log 2>&1 &

# Give monchaind node enough time to launch
sleep 5
...
#PRIVKEY="$("$PWD"/build/monchaind keys export $KEY)"
...
# After tests
# kill test monchaind
echo "going to shutdown monchaind in 3 seconds..."
sleep 3
pkill -f "ethermint*"