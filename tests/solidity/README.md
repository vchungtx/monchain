...
**Prerequisite**: in the repo's root, run `make install` to install the `monchaind` and `monchaind` binaries. When done, come back to this directory.
...
To run the tests, you can use the `test-helper.js` utility to test all suites under `ganache` or `ethermint` network. The `test-helper.js` will help you spawn an `monchaind` process before running the tests.
...
If you need to take more control, you can also run `monchaind` using:
...
Keep the terminal window open, go into any of the tests and run `yarn test-ethermint`. You should see `monchaind` accepting transactions and producing blocks. You should be able to query for any transaction via:

- `monchaind query tx <cosmos-sdk tx>`
...
Running `monchaind keys list --keyring-backend=test` should output:
