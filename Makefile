...
docker-build:
	...
	mkdir -p ./build/
	docker cp ethermint:/usr/bin/monchaind ./build/
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(ETHERMINT_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\ethermint\Z monchaind/node "./monchaind testnet --v 4 -o /ethermint --keyring-backend=test --ip-addresses monchaindnode0,monchaindnode1,monchaindnode2,monchaindnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(ETHERMINT_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/ethermint:Z monchaind/node "./monchaind testnet --v 4 -o /ethermint --keyring-backend=test --ip-addresses monchaindnode0,monchaindnode1,monchaindnode2,monchaindnode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\ethermitd:ethermint\Z monchaind/node "./monchaind unsafe-reset-all --home=/ethermint"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\ethermitd:ethermint\Z monchaind/node "./monchaind unsafe-reset-all --home=/ethermint"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\ethermitd:ethermint\Z monchaind/node "./monchaind unsafe-reset-all --home=/ethermint"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\ethermitd:ethermint\Z monchaind/node "./monchaind unsafe-reset-all --home=/ethermint"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/ethermitd:/ethermint:Z monchaind/node "./monchaind unsafe-reset-all --home=/ethermint"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/ethermitd:/ethermint:Z monchaind/node "./monchaind unsafe-reset-all --home=/ethermint"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/ethermitd:/ethermint:Z monchaind/node "./monchaind unsafe-reset-all --home=/ethermint"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/ethermitd:/ethermint:Z monchaind/node "./monchaind unsafe-reset-all --home=/ethermint"
endif
